...
namespace Winsoft.Common
{
    public class FTPClientService
    {
        ...
        public FTPClientService(string remoteHost, string remotePath, string remoteUser, string remotePass, int remotePort)
        {
        ...
        public void Connect()
        {
            ...
            IPEndPoint ep = new IPEndPoint(System.Net.IPAddress.Parse(strRemoteHost), strRemotePort);
            // 链接
            try
            {
        ...
        public void Get(string strFileNameMask, string strFolder)
        {
            ...
            {
                if (!strFile.Equals(""))//一般来说strFiles的最 后一个元素可能是空字符串
                {
        ...
        private Socket CreateDataSocket()
        {
            ...
            IPEndPoint ep = new IPEndPoint(System.Net.IPAddress.Parse(ipAddress), port);
            try
            {
                s.Connect(ep);
            }
        ...
        private string ReadLine()
        {
            ...
            if (strMsg.Length > 2)
            {
                ...
                //所以最后一个mess是没用的空字符串
                //但为什么不直接取mess[0],因为只有最后一行字 符串应答码与信息之间有空格
            }
            else
            {
                strMsg = mess[0];
